...
app.post('/login',(req,res)=>{
    ...
    if(username && password){
        pool.query('select * from user where username =? AND password=?',[username,password],(err,row)=>{
            ...
            if(row.length>0){
               ...
               // console.log(row[0]['username'] + " " + row[0]['email'])
                res.send({data:{id:row[0].id,username:row[0].username,email:row[0].email},token:token})
            }else{
...
// const {username,email}=req.body;
 const user={
    ...
    const file=req.file;
    let sql='insert into user(username, password , email ,images) values(?,?,?,?)';
     pool.query(sql,[user.username,hashPass,user.email,JSON.stringify(file)],(err,row)=>{
         if(err){
            res.status(500).send(err)
         } else{
             logger.log('info',{"newUser":{username:user.username,email:user.email}})
             res.status(200).send({"newUser":{username:user.username,email:user.email}})
         }
